    ...
    if not (inFramecheck):
        ...
        if resl.pose_landmarks and resr.pose_landmarks is not None:
            if (endT - startT) <= dur:
                ...
                nposl = abs(int(resl.pose_landmarks.landmark[28].y * 100))
                print('original:', nposl)
                nposr = abs(int(resr.pose_landmarks.landmark[28].y * 100))
                ...
                if (cEnd - cStart) <= 3:
                    ...
                    isInit = False
                    isCinit = False
    cv2.circle(left, (172, 408), 15, (0, 0, 255), -1)
